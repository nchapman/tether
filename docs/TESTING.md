...
| `tether-input` | 15 | Modifier tracking, HID routing, cursor normalization, `clamp_relative_delta` (±1000 px per event). On Linux, `inject/uinput.rs` adds HID→evdev mapping, ASCII char→keystroke, scroll-detent sign/quantisation, and normalised→ABS coordinate unit tests; the macOS/Windows clamp tests live in `inject/enigo_backend.rs`. |
