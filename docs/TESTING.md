...
| `tether-codec` | 10 default + 8 `#[ignore]` Linux + 9 `#[ignore]` macOS | Negotiation preference order (5-entry list, 10-bit ranking), `validate_chosen_profile` accepts / rejects / empty-advertised paths, `pick_supported_profile` forward-compat on unknown `bit_depth`, fixture-presence cross-check, decode-probe fixture map for all of HEVC 4:2:0/4:4:4 × 8/10-bit + H.264 4:2:0 8-bit; SW H264 round-trip (test-only); VT `codec_name` map; VAAPI encoder/decoder/dma-buf-import on Linux hardware; per-codec × per-resolution benchmarks (`vaapi::bench`, 4 cells × 3 paths); VideoToolbox: encoder construct, H.264 BGRA round-trip, HEVC 10-bit / 4:4:4 probe matrix, `videotoolbox_round_trip_chroma_matrix` cross-checks the probe's encode bit against an independent encode→decode→IOSurface-fourcc check (catches silent downsample regressions), self-decodable-IDR mid-session recovery. |
