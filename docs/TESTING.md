...
- Hardware tests (VAAPI/Vulkan/Metal/D3D11) are **not** run in CI — GitHub-hosted
  ...
  knob SKIP tests). The Windows D3D11 render matrix now covers the same shapes
  it can (HEVC + H.264 coord/colour at 8/10-bit, client-upscale, surface-below-
  video); its remaining gaps are AV1 (needs Arc-class encode) and 4:4:4 (no
  D3D11 VP path), both hardware/capability limits rather than missing tests. The
  macOS VideoToolbox/IOSurface matrix is the next to broaden. Closing those is
