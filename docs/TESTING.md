...
| `tether-host` | app unit tests | Cursor tick emission, session-lifecycle Arc drop invariant, host audio summary gating, viewport/resize policy and duplicate filtering, client display-metrics dedupe, bitrate derivation policy, BGRA resize helpers, macOS fourcc table consistency, and Linux `--setup-input` manual fallback text/rule embedding. |
| `tether-client` | app unit tests | CLI parsing incl. `--pin` / `--label` / `--no-audio`, fingerprint hex parsing, decode-recovery trigger policy, startup viewport drain, viewport/display-metrics send dedupe, and Windows D3D11 decode→render format-table consistency. |
