...
| `tether-host` | app unit tests | Cursor tick emission, session-lifecycle Arc drop invariant, host audio summary gating, viewport/resize policy and duplicate filtering incl. the `Original` no-cap sentinel, client display-metrics dedupe, bitrate derivation policy, BGRA resize helpers, macOS fourcc table consistency, and Linux `--setup-input` manual fallback text/rule embedding. |
| `tether-client` | app unit tests | CLI parsing incl. `--pin` / `--label` / `--no-audio` / `--view-mode`, fingerprint hex parsing, decode-recovery trigger policy, startup viewport drain, viewport/display-metrics send dedupe, `Original` view-mode no-cap viewport behavior plus no-`SetDisplayMode` invariant and renderer presentation-mode selection, and Windows D3D11 decode→render format-table consistency. |
