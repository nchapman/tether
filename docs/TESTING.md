...
| `tether-client` | app unit tests | CLI parsing incl. `--pin` / `--label` / `--no-audio` / `--view-mode`, fingerprint hex parsing, decode-recovery trigger policy, startup viewport drain, viewport/display-metrics send dedupe, `Original` view-mode no-cap viewport behavior plus no-`SetDisplayMode` invariant, and Windows D3D11 decode→render format-table consistency. |
